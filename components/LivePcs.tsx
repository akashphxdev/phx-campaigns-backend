...
//   // avg time — recent_avg_seconds (seconds) ya avg_time (minutes)
//   const avgMs = sys.recent_avg_seconds != null
//     ? sys.recent_avg_seconds * 1000
//     : (sys.avg_time ?? 3) * 60 * 1000

//   // offline threshold = avg time * 2
//   const OFFLINE_THRESH_MS = avgMs * 2

//   if (now - lastGetMs > OFFLINE_THRESH_MS) {
//     return { status: 'offline', lastProcessMs }
//   }
...
function getStatus(sys: SystemRow): { status: SystemStatus; lastProcessMs: number | null } {
  ...
  // Offline threshold — always from avg_time (systems table), fallback 3 min
  const offlineAvgMs = (sys.avg_time ?? 3) * 60 * 1000
  const OFFLINE_THRESH_MS = offlineAvgMs * 2
  ...
  // Fast/Slow — use recent_avg_seconds (last 5 jobs) for comparison
  const speedAvgMs = sys.recent_avg_seconds != null
    ? sys.recent_avg_seconds * 1000
    : offlineAvgMs

  const status: SystemStatus = lastProcessMs < speedAvgMs ? 'fast' : 'slow'
  return { status, lastProcessMs }
}
