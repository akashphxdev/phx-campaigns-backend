...
export async function GET(req: NextRequest) {
  try {
    ...
    // 8. CSV row → processing mark karo + started_at update
    await db.query(
      `UPDATE csv_rows
    ...
    // 9. System last_req + last_get_at update
    await db.query(
      `UPDATE systems SET last_req = NOW(), last_get_at = NOW() WHERE id = ?`,
      [system_id]
    )

    // 10. Campaign image fetch — system ki campaign_id se pending image lo
...
export async function POST(req: NextRequest) {
  try {
    ...
    // System stats update — avg_time nahi chhena
    if (status === 'success') {
      await db.query(
        `UPDATE systems
